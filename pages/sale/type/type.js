...
Page({
  ...
  watchWeight: function (event) {
    ...
    if (_this.data.price != ""&&_this.data.totalWeight!='') {
      _this.setData({
        totalPrice: (parseFloat(_this.data.totalWeight) * parseFloat(_this.data.price)).toFixed(2)
      })
    }
  },
  ...
  formSubmit(e) {
    if (_this.data.list.length > 0) {
      ...
      for (var i = 0; i < _this.data.list.length; i++) {
        ...
        if (_this.data.typeId == typeId) {
          ...
          if(_this.data.id!=''&&_this.data.id!=_this.data.list[i].id){
            isHas = true;
          }
         }
      }
    ...
    var content = {
      id: String(id),
      saleTypeId:_this.data.saleTypeId,
      ...
      pages: [{
        ...
        },
        {
          id: '',
          name: '总价',
          value: e.detail.value.totalPrice
        }
      ]
    };
    ...
    if (pages.length > 1) {
      ...
      for (var i = 0; i < _this.data.list.length; i++) {
        var pages = _this.data.list[i].pages;
        totalCount+=parseFloat(pages[1].value);
        totalWeight+=parseFloat(pages[2].value);
        totalPrice+=parseFloat(pages[6].value);
      }
      prevPage.setData({
